...
class nPuzzler
{
	...
	public static void main(String[] args)
	{
		...
		else
		{
			...
			//Animate solution;
			PuzzleState current = gPuzzle.StartState;
			current.print();
			for(int j = 0; j < thisSolution.length; j++) {
				System.out.println();
				System.out.println("Press Any Key To Continue...");
				new java.util.Scanner(System.in).nextLine();
				// TODO: Insert CLS here
				
				current = current.move(thisSolution[j]);
				current.print();
			}
			*/
		}
	...
	private static nPuzzle readProblemFile(String fileName) // this allow only one puzzle to be specified in a problem file 
	{
		...
		try
		{
			...
			int[][] startPuzzleGrid = new int[puzzleSizeX][puzzleSizeY];
			int[][] goalPuzzleGrid = new int[puzzleSizeX][puzzleSizeY];
			
			//fill in the start state
			String startStateString = puzzle.readLine();
			startPuzzleGrid = ParseStateString(startStateString, startPuzzleGrid, puzzleSizeX);
			
			//fill in the end state
			String goalStateString = puzzle.readLine();
			goalPuzzleGrid = ParseStateString(goalStateString, goalPuzzleGrid, puzzleSizeX);
